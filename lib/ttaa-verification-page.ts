...
export function ttaaVerificationDocumentHtml(document: TtaaVerificationDocument, marker: string) {
  ...
  if (document.fileKey !== undefined && !/^ttaa\/[a-f0-9-]{36}\/[a-z0-9-]+\.pdf$/.test(document.fileKey)) throw new Error("The private PDF record is invalid.");
  const token = /^TTAA_VERIFICATION:([a-f0-9-]{36})$/.exec(marker)?.[1];
  if (!token) throw new Error("The verification page token is invalid.");
  const number = escapeHtml(document.documentNumber);
  const hasFile = Boolean(document.fileKey || url);
  const viewerUrl = hasFile ? escapeHtml(verificationPdfViewerUrl("ttaa", token)) : "";
  const fileSection = hasFile
    ? `<section class="ayv-file" aria-labelledby="ttaa-file-title"><div class="ayv-file-head"><div><span class="ayv-section-label">03 · DOCUMENT FILE</span><h2 id="ttaa-file-title">Verified document</h2><p>The file is shown only inside the protected viewer and is excluded from search indexing.</p></div></div><div class="ayv-pdf-frame"><div class="ayv-pdf-bar">Protected document preview <small>PDF</small></div><iframe class="ayv-pdf" src="${viewerUrl}" title="Verified document ${number}" loading="lazy" referrerpolicy="no-referrer"></iframe></div></section>`

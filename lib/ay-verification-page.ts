...
export function ayVerificationSeo(documentNumber?: string) {
  ...
  return {
    ...
    description: `AY Tercüme tarafından doğrulanan ${documentNumber.slice(0, 30)} numaralı belgeyi ve dosyayı görüntüleyin. Ankara ve İstanbul ofislerimize ulaşın.`,
  };
}
...
export function ayVerificationDocumentHtml(document: AyVerificationDocument, marker: string) {
  ...
  if (url.protocol !== "https:" || !["aytercume.com", "www.aytercume.com"].includes(url.hostname)) throw new Error("Belge bağlantısı aytercume.com üzerinde HTTPS olmalıdır.");
  const number = escapeHtml(document.documentNumber);
  const kind = escapeHtml(document.documentType);
  const fileUrl = escapeHtml(url.toString());
  return `${style}<main class="ayv"><header class="ayv-hero"><span class="ayv-kicker">AY TERCÜME · BELGE DOĞRULAMA</span><span class="ayv-check" aria-hidden="true">✓</span><h1>Bu dosya AY Tercüme tarafından doğrulanmıştır.</h1><p class="ayv-lead">Aşağıdaki belge bilgilerini ve dosyanın kendisini inceleyebilirsiniz.</p><span class="ayv-status">Doğrulanmış belge</span></header>

...
import { createTtaaVerificationLabel, validatePrototypeDetails, type PrototypeDetails } from "../../lib/qr-prototype";

type Label = Awaited<ReturnType<typeof createTtaaVerificationLabel>>;
type Result = { details: PrototypeDetails; label: Label; pageUrl: string; file: File | null; hasFile: boolean };
type Found = { url: string; details: Pick<PrototypeDetails, "documentNumber" | "customer" | "documentDate" | "documentType">; hasFile: boolean; storage: "private" | "wordpress" | "none" };
type LegacyRecord = { id: string; details: PrototypeDetails; hasFile: boolean; pdfName: string | null };
const documentTypes = ["Translation document", "Diploma / education document", "Official document", "Contract", "Other"];
...
async function pngFromSvg(url: string) {
  ...
  if (!context) throw new Error("PNG QR kodu hazırlanamadı.");
...
export default function TtaaVerificationStudio({ today }: { today: string }) {
  const blank: PrototypeDetails = { documentNumber: "", customer: "", documentType: "Translation document", documentDate: today, driveLink: "" };
  ...
  const [legacyBusyId, setLegacyBusyId] = useState("");
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const response = await fetch(`/api/qr-documents/ttaa/verification?list=1&q=${encodeURIComponent(listQuery)}&page=${listPage}`, { cache: "no-store", signal: controller.signal });
  ...
  useEffect(() => {
    ...
    void fetch("/api/qr-documents/ttaa?list=1", { cache: "no-store", signal: controller.signal }).then(async (response) => {
      const payload = await response.json() as { records?: LegacyRecord[]; error?: string };
      if (!response.ok) throw new Error(payload.error || "Önceki TTAA kayıtları okunamadı.");
      setLegacyRecords(payload.records || []);
    }).catch((failure) => { if (!controller.signal.aborted) setLegacyError(failure instanceof Error ? failure.message : "Önceki TTAA kayıtları okunamadı."); });
    return () => controller.abort();
  }, []);

  async function selectRecord(entry: Found) {
    const details = { ...entry.details, driveLink: "" };
    const label = await createTtaaVerificationLabel(details, entry.url);
  ...
  async function importLegacy(entry: LegacyRecord) {
    if (legacyBusyId || !window.confirm(`${entry.details.documentNumber} numaralı eski kaydın müşteri adı TTAA doğrulama sayfasında görünecek; varsa PDF özel depoda saklanıp güvenli görüntüleyicide açılacak. Eski basılı prototip QR yönlendirmez; yeni QR kodunu yazdırmanız gerekir. Devam edilsin mi?`)) return;
    setLegacyBusyId(entry.id); setLegacyError("");
    try {
      const data = new FormData();
      data.set("mode", "import"); data.set("documentNumber", entry.details.documentNumber); data.set("confirmed", "true");
      const response = await fetch("/api/qr-documents/ttaa/verification", { method: "POST", body: data });
      const payload = await response.json() as { url?: string; hasFile?: boolean; error?: string; warning?: string | null };
      if (!response.ok && response.status !== 409) throw new Error(payload.error || "Eski kayıt aktarılamadı.");
      if (!payload.url) throw new Error(payload.error || "Doğrulama sayfası bulunamadı.");
      await selectRecord({ url: payload.url, details: entry.details, hasFile: Boolean(payload.hasFile), storage: payload.hasFile ? "private" : "none" });
      if (response.status === 409 && entry.hasFile && !payload.hasFile) {
        setLegacyError("Sayfa zaten mevcut fakat eski kaydın PDF’si bu sayfaya aktarılmadı. PDF yönetiminden dosyayı ayrıca ekleyin.");
      } else {
        setNotice(payload.warning ? `Resmî QR hazır. ${payload.warning}` : "Resmî doğrulama sayfası ve yeni QR hazır. Eski basılı QR’ların yerine bu QR kodunu kullanın.");
      }
      setListReload((value) => value + 1);
    } catch (failure) { setLegacyError(failure instanceof Error ? failure.message : "Eski kayıt aktarılamadı."); }
    finally { setLegacyBusyId(""); }
  }
  ...
  async function generate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFound({ details, url: payload.url, hasFile: Boolean(payload.hasFile), storage: payload.hasFile ? "private" : "none" });
  ...
  async function lookupDocument(event: FormEvent<HTMLFormElement>) {
    ...
    if (lookupFileInput.current) lookupFileInput.current.value = "";
    try {
      const response = await fetch(`/api/qr-documents/ttaa/verification?documentNumber=${encodeURIComponent(lookupNumber.trim())}`, { cache: "no-store" });
  ...
  async function attachDocument(event: FormEvent<HTMLFormElement>) {
    ...
    if (!attachConfirmed) { setLookupError("PDF’nin doğrulama sayfasında güvenli görüntüleyiciyle gösterileceğini onaylayın."); return; }
    const data = new FormData();
    ...
    try {
      ...
      setFound({ ...found, hasFile: true, storage: "private" });
  ...
  async function removeDocument() {
    ...
    try {
      ...
      setFound({ ...found, hasFile: false, storage: "none" });
  ...
  async function deleteRecord() {
    if (!found || attachBusy) return;
    const confirmation = window.prompt(`Bu işlem WordPress doğrulama sayfasını ve bağlı PDF dosyasını kalıcı olarak siler. Onaylamak için belge numarasını yazın:\n\n${found.details.documentNumber}`);
    if (confirmation === null) return;
    if (confirmation.trim() !== found.details.documentNumber) { setLookupError("Belge numarası eşleşmedi; kayıt silinmedi."); return; }
    setAttachBusy(true); setLookupError(""); setLookupNotice("");
    try {
      const data = new FormData(); data.set("mode", "delete"); data.set("documentNumber", found.details.documentNumber); data.set("deleteConfirmation", confirmation.trim());
      const response = await fetch("/api/qr-documents/ttaa/verification", { method: "POST", body: data });
      const payload = await response.json() as { error?: string; deleted?: boolean; warning?: string | null };
      if (!response.ok || !payload.deleted) throw new Error(payload.error || "Doğrulama kaydı silinemedi.");
      const deletedNumber = found.details.documentNumber;
      reset(); setFound(null); setLookupNumber(""); setLookupFile(null); setAttachConfirmed(false);
      if (lookupFileInput.current) lookupFileInput.current.value = "";
      setLookupNotice(payload.warning ? `${deletedNumber} silindi. ${payload.warning}` : `${deletedNumber} numaralı doğrulama kaydı ve sayfası silindi.`);
      setListReload((value) => value + 1);
    } catch (failure) { setLookupError(failure instanceof Error ? failure.message : "Doğrulama kaydı silinemedi."); }
    finally { setAttachBusy(false); }
  }

  async function secureLegacyPdf() {
    if (!found || found.storage !== "wordpress" || attachBusy) return;
    setAttachBusy(true); setLookupError(""); setLookupNotice("");
    try {
      const data = new FormData(); data.set("mode", "secure"); data.set("documentNumber", found.details.documentNumber); data.set("confirmed", "true");
      const response = await fetch("/api/qr-documents/ttaa/verification", { method: "POST", body: data });
      const payload = await response.json() as { error?: string; url?: string; warning?: string | null };
      if (!response.ok || payload.url !== found.url) throw new Error(payload.error || "PDF güvenli depoya taşınamadı.");
      setFound({ ...found, storage: "private" });
      setLookupNotice(payload.warning ? `PDF güvenli depoya taşındı. ${payload.warning}` : "PDF özel depoya taşındı; açık WordPress medya adresi kaldırıldı.");
      setListReload((value) => value + 1);
    } catch (failure) { setLookupError(failure instanceof Error ? failure.message : "PDF güvenli depoya taşınamadı."); }
    finally { setAttachBusy(false); }
  }
  ...
  async function createLanding() {
    ...
    try {
      const response = await fetch("/api/qr-documents/ttaa/verification/landing", { method: "POST" });
  ...
  async function downloadPng() {
    ...
    try { downloadFile(await pngFromSvg(result.label.labelUrl), `ttaa-${result.details.documentNumber}-qr.png`); }
    catch { setError("PNG indirilemedi. SVG seçeneğini deneyin."); }
    finally { setBusy(false); }
  }

  return <>
    <div className="qr-proto-heading"><div><p className="qr-eyebrow">TTAA · BELGE YÖNETİMİ</p><h1>QR Dosya Doğrulama</h1><p>Belgeyi kaydedin, turkishtranslation.com.tr doğrulama sayfasını ve QR kodunu oluşturun.</p></div><span className="qr-live-status"><i /> WordPress bağlantılı</span></div>
    <div className="qr-proto-notice qr-public-notice"><span aria-hidden="true">i</span><p>PDF olmadan da belgeye özel sayfa ve QR oluşur. PDF eklendiğinde özel depoda saklanır, arama motorlarına kapatılır ve yalnızca doğrulama sayfasındaki güvenli görüntüleyicide gösterilir. Müşteri adı doğrulama sayfasında görünür.</p></div>
    ...
    <div className="qr-proto-grid">
      <section className="qr-form-panel" aria-labelledby="qr-form-title">
        ...
        <form onSubmit={generate}>
          <fieldset disabled={busy}>
            ...
            <label>Belge türü<select value={form.documentType} onChange={(event) => change("documentType", event.target.value)}>{!documentTypes.includes(form.documentType) ? <option>{form.documentType}</option> : null}{documentTypes.map((type) => <option key={type}>{type}</option>)}</select></label>
            ...
            <label className="qr-confirm"><input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} required /><span>Bu belgeyi kontrol ettim. Müşteri adının görüneceğini ve eklersem PDF’nin güvenli görüntüleyicide açılacağını kabul ediyorum.</span></label>
          </fieldset>
      ...
      <section className="qr-label-panel" aria-labelledby="qr-label-title">
        <div className="qr-panel-heading"><div><small>02 · QR KOD</small><h2 id="qr-label-title">Yalnızca QR kod</h2></div><span className="qr-size-badge">Kare çıktı</span></div>
        <div className="qr-label-stage">{result ? <img className="qr-generated-label" src={result.label.labelUrl} alt={`TTAA belge ${result.details.documentNumber} QR kodu`} /> : <div className="qr-label-empty"><span><QrIcon size={48} /></span><strong>QR kodu burada görünecek</strong><p>Belge bilgilerini kaydettiğinizde<br />WordPress sayfası oluşturulur.</p></div>}</div>
        <div aria-live="polite" className={`qr-label-feedback${dirty ? " is-dirty" : ""}`}>{dirty ? "Bilgiler değişti. Mevcut QR kod önce yayımlanan sayfaya aittir; yeni kayıt oluşturmadan önce alanları kontrol edin." : notice || "QR doğrudan turkishtranslation.com.tr üzerindeki belge sayfasına gider."}</div>
        ...
        <div className="qr-label-actions"><button type="button" className="qr-primary-button" disabled={!ready} onClick={() => window.print()}>QR kodu yazdır</button><button type="button" className="qr-secondary-button" disabled={!ready} onClick={() => void downloadPng()}>QR PNG indir</button><button type="button" className="qr-secondary-button" disabled={!ready} onClick={() => { if (result) downloadFile(result.label.labelUrl, `ttaa-${result.details.documentNumber}-qr.svg`); }}>QR SVG indir</button></div>
        <p className="qr-print-help">Çıktı yalnızca QR koddan oluşur. PNG veya SVG dosyasını kendi şablonunuza doğrudan ekleyebilirsiniz.</p>
      </section>
    </div>

    <section className="qr-demo-list" aria-labelledby="qr-records-title"><div className="qr-list-heading"><div><h2 id="qr-records-title">Belge kayıtları <span>{records.length}</span></h2><p>Önceden yayımlanan TTAA doğrulama sayfaları da burada görünür.</p></div><div className="qr-list-tools"><input aria-label="Belge kayıtlarında ara" placeholder="Belge no veya müşteri ara" value={listQuery} onChange={(event) => { setListQuery(event.target.value); setListPage(1); }} /><button type="button" className="qr-secondary-button" onClick={() => setListReload((value) => value + 1)} disabled={listBusy}>Yenile</button></div></div>
      {listError ? <p className="qr-form-error" role="alert">{listError}</p> : null}
      {listBusy ? <p className="qr-list-message">Kayıtlar yükleniyor…</p> : records.length ? <div className="qr-table-scroll"><table><thead><tr><th>Belge no</th><th>Müşteri</th><th>Belge türü</th><th>PDF</th><th>Sayfa / QR</th><th>İşlem</th></tr></thead><tbody>{records.map((entry) => <tr key={entry.url} className={found?.url === entry.url ? "is-selected" : ""}><td><strong>{entry.details.documentNumber}</strong><small>{entry.details.documentDate.split("-").reverse().join(".")}</small></td><td>{entry.details.customer}</td><td>{entry.details.documentType}</td><td><span className={`qr-record-status${entry.storage === "private" ? " is-complete" : ""}`}>{entry.storage === "private" ? "Güvenli PDF" : entry.storage === "wordpress" ? "Eski açık PDF" : "PDF bekleniyor"}</span></td><td><a href={entry.url} target="_blank" rel="noopener noreferrer">Sayfayı aç ↗</a></td><td><button type="button" className="qr-text-button" onClick={() => void selectRecord(entry)}>Kaydı / QR’ı aç</button></td></tr>)}</tbody></table></div> : <p className="qr-list-message">{listQuery ? "Aramayla eşleşen kayıt bulunamadı." : "Bu sayfada kayıt yok."}</p>}
    ...
    <section className="qr-update-card" aria-labelledby="qr-update-title"><div className="qr-panel-heading"><div><small>03 · PDF YÖNETİMİ</small><h2 id="qr-update-title">Kayıtlı belgenin PDF’si</h2></div></div><p className="qr-update-intro">Listeden bir kayıt açın veya belge numarasıyla arayın. PDF ekleyebilir, değiştirebilir veya kaldırabilirsiniz; basılmış QR adresi aynı kalır.</p>
      ...
      {found ? <div className="qr-found-record"><div><small>MEVCUT WORDPRESS SAYFASI</small><strong>{found.details.documentNumber} · {found.details.customer}</strong><span>{found.details.documentDate.split("-").reverse().join(".")} · {found.details.documentType}</span><a href={found.url} target="_blank" rel="noopener noreferrer">Doğrulama sayfasını aç ↗</a>{found.storage === "wordpress" ? <button type="button" className="qr-primary-button" disabled={attachBusy || designBusy} onClick={() => void secureLegacyPdf()}>{attachBusy ? "Taşınıyor…" : "PDF’yi güvenli depoya taşı"}</button> : null}<button type="button" className="qr-text-button" disabled={designBusy || attachBusy} onClick={() => void refreshDesign()}>{designBusy ? "Tasarım güncelleniyor…" : "Sayfa tasarımını yenile"}</button><button type="button" className="qr-danger-button qr-delete-record" disabled={attachBusy || designBusy} onClick={() => void deleteRecord()}>{attachBusy ? "İşlem sürüyor…" : "Doğrulama kaydını ve sayfayı sil"}</button></div><span className={`qr-record-status${found.storage === "private" ? " is-complete" : ""}`}>{found.storage === "private" ? "Güvenli PDF" : found.storage === "wordpress" ? "Eski açık PDF" : "PDF bekleniyor"}</span></div> : null}
      {found ? <form className="qr-attach-form" onSubmit={attachDocument}><label>{found.hasFile ? "Yerine yüklenecek yeni PDF" : "Bu kayda eklenecek PDF"} <span>*</span><input ref={lookupFileInput} className="qr-file-input" type="file" accept=".pdf,application/pdf" required onChange={(event) => { setLookupFile(event.target.files?.[0] || null); setLookupError(""); }} /><small>En fazla 8 MB. Özel depoda saklanır ve yalnızca güvenli görüntüleyicide gösterilir.</small></label><label className="qr-confirm"><input type="checkbox" checked={attachConfirmed} onChange={(event) => setAttachConfirmed(event.target.checked)} required /><span>Bu PDF’nin doğru belgeye ait olduğunu ve doğrulama sayfasına erişen kişilerce görüntülenebileceğini onaylıyorum.</span></label><div className="qr-form-actions"><button type="submit" className="qr-primary-button" disabled={attachBusy}>{attachBusy ? "PDF kaydediliyor…" : found.hasFile ? "PDF’yi değiştir" : "PDF ekle"}</button>{found.hasFile ? <button type="button" className="qr-danger-button" disabled={attachBusy} onClick={() => void removeDocument()}>PDF’yi kaldır</button> : null}</div></form> : null}

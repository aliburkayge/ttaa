...
test("TTAA PDF add, replace and remove retain the original QR URL", async (t) => {
  ...
  const token = ttaaVerificationToken(document.documentNumber);
  const first = await publishTtaaVerificationDocument(document, token);
  const pdf1 = `ttaa/${token}/11111111-2222-3333-4444-555555555555.pdf`;
  const attached = await attachTtaaVerificationPdf(token, pdf1);
  ...
  const pdf2 = `ttaa/${token}/22222222-3333-4444-5555-666666666666.pdf`;
  const replaced = await setTtaaVerificationPdf(token, { key: pdf2 });
  assert.equal(replaced.url, first.url);
  assert.equal(replaced.document.fileKey, pdf2);
  assert.equal(replaced.document.fileUrl, undefined);
